/**
 * Sign In Background - WebGL Liquid Gradient + Halftone Dither
 * Duplicated from research-hero-background.js so sign-in-specific tweaks
...
class LiquidGradientEffect {
  constructor(containerId = 'webgl-background-sign-in') {
  ...
  createControls() {
    const panel = document.createElement('div');
      panel.style.cssText = `
        ...
        font-size: 13px; max-width: 280px; max-height: calc(100vh - 90px); overflow-y: auto;
      `;
    ...
    const waveAmpLabel = makeLabel('Wave Amount');
    waveAmpLabel.appendChild(makeRange(0, 1, 0.01, this.config.waveAmp, (v) => {
      this.config.waveAmp = v;
      this.uniformsLiquid.waveAmp.value = v;
    }));

    const waveFreqLabel = makeLabel('Wave Frequency');
    waveFreqLabel.appendChild(makeRange(0, 30, 0.5, this.config.waveFreq, (v) => {
      this.config.waveFreq = v;
      this.uniformsLiquid.waveFreq.value = v;
    }));

    const waveRotationLabel = makeLabel('Wave Rotation (radians)');
    waveRotationLabel.appendChild(makeRange(0, 6.283, 0.01, this.config.waveRotation, (v) => {
      this.config.waveRotation = v;
      this.uniformsLiquid.waveRotation.value = v;
    }));
    ...
    liquidSection.content.appendChild(waveAmpLabel);
    liquidSection.content.appendChild(waveFreqLabel);
    liquidSection.content.appendChild(waveRotationLabel);
...
function init() {
  const ids = new Set();
  const variantContainers = document.querySelectorAll('[data-webgl-variant="sign-in"]');
  ...
  if (ids.size === 0) {
    ids.add('webgl-background-sign-in');
  }
